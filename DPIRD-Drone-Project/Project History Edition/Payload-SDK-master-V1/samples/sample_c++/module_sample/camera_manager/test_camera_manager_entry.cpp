...
/* Keep the complete `list[]` structure for PSDK compatibility */
...
/* **Absolute Mapping Table**: Ensure input `1-4` maps to the correct `enum` */
...
/* Directly enter NO.2 gimbal camera */
void DjiUser_RunCameraManagerSample(void)
{
    T_DjiOsalHandler *osalHandler = DjiPlatform_GetOsalHandler();
    string sampleSelectStr;
    int sampleSelectNum;
    E_DjiMountPosition cameraMountPosition = DJI_MOUNT_POSITION_PAYLOAD_PORT_NO1; // Fixed to NO.2 gimbal
    ...
    while (true) {
        ...
        cout << "\nAvailable samples:\n";
        DjiTest_CmareManagerShowStringList(s_cameraManagerSampleSelectList, 4);
        ...
        sampleSelectNum = atoi(sampleSelectStr.c_str()) - 1; // Start from index 1

        if (sampleSelectNum < 0 || sampleSelectNum >= 4) {
            USER_LOG_ERROR("Input camera sample is invalid");
            continue;
        }

        // ? **Use absolute mapping to ensure `1-4` corresponds correctly to the `enum`**
